...
export async function loader({ request }: LoaderFunctionArgs) {
  try {
    ...
    console.error("admin.rentas._index loader error:", error);
    return json({ bookings: [] });
  }
}
...
export async function action({ request }: ActionFunctionArgs) {
  try {
    ...
    console.error("admin.rentas._index action error:", error);
    return json({ error: "Error al actualizar la reservación. Intente de nuevo." }, { status: 500 });
  }
}

export default function AdminRentalsIndex() {
  ...
  return (
    <div>
      ...
      <div className="bg-white rounded-xl border border-obsidian-200 shadow-sm overflow-hidden">
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-obsidian-200 bg-obsidian-50">
                ...
                <th className="text-left px-6 py-3 text-xs font-heading font-semibold text-obsidian-500 uppercase tracking-wider">Barril</th>
            ...
            <tbody className="divide-y divide-obsidian-100">
              {bookings.map((b) => (
                <tr key={b.id} className="hover:bg-obsidian-50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-sm text-obsidian-600">{b.product.name}</td>
                  <td className="px-6 py-4">
                    <p className="text-sm text-obsidian-600">{formatDateShort(b.eventDate)}</p>
                    <p className="text-xs text-obsidian-400">{b.guestCount} invitados</p>
                  </td>
        ...
        {bookings.length === 0 && (
          <div className="p-8 text-center text-obsidian-500">No hay reservaciones de renta.</div>
        )}
      </div>
    </div>
  );
